"""
    Home Task 5: Dictionaries, Search, Sorting.
...
# retrieving elements according to the search criteria,
# adding them to the new dictionary with results.

for car, detail in car_data.items():
    if all((detail[1] >= search_criteria[0],
            detail[2] >= search_criteria[1],
            detail[4] <= search_criteria[2],
            x < 5)):
        x += 1
        final_dict_of_cars[car] = detail

# printing the outcome with the sorted list
print(sorted(final_dict_of_cars.items(), key=lambda y: y[1][4]))
